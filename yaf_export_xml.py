...
from yaf_texture_xml import yafTexture
...
class yafrayRender:
	...
	def processMaterialTextures(self, mat, job_id):
		if mat:
			if mat.properties['YafRay']['type'] == 'blend':
				...
				for material in [mat1, mat2]:
					self.processMaterialTextures(material, job_id)
				self.exportMaterialTextures(mat, job_id)
			else:
				self.exportMaterialTextures(mat, job_id)


	def exportMaterialTextures(self, mat, job_id):
		...
		if hasattr(mat, 'enabledTextures'):
			...
			for m in used:
				...
				self.yTexture.writeTexture(tex, tname, job_id, mat.lib, self.inputGamma)
	...
	def processObjectTextures(self, mesh_object, job_id):
		isVolume = False
		try:
		...
		for mat in mesh_object.getData().getMaterials():
			if isVolume:
				objProp["noise_tex"] = self.exportMaterialTextures(mat, job_id)
			else:
	...
	def exportTextures(self, job_id):
		self.yi.printInfo("Exporter: Processing Textures...")
		self.textures = set()
		for o in self.objects:
			if self.isMesh(o):
				self.processObjectTextures(o, job_id)
		for o in self.instanced:
			if self.isMesh(o):
				self.processObjectTextures(o, job_id)
	...
	def render(self, job_id, viewRender = False):
	...
	def renderAnim(self, job_id, aBreak=True):
		render = self.scene.getRenderingContext()
